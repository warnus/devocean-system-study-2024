...
- 데이터센터가 필요한 시점은?
- 메시지 큐가 보장하는 기본 원칙은?
- 메시지 큐가 있을때 해결해주는 장애상황?
- 로그, 매트릭, 자동화를 하는 이유?
- DB 규모확장이 필요할 때는?
- DB scaleup 한계?
- DB sharding 문제점 3가지와 해결방법 제안?
...
- 로그
	- 에러를 손쉽게 찾기 위해
	- 로그를 단일 서비스로 모아주는 도구를 사용하면 편리
- 메트릭: 사업 현황과 같은 유용한 정보
	- 호스트단위(CPU, 메모리, 디스크/IO)
	- 종합(aggregated/DB성능, 캐시성능)
	- 핵심비즈니스(Daily Active User, 수익/Revinue, Retention/재방문)
- 자동화
	- 코드 검증절차를 자동으로
	- 시스템이 크고 복잡해지면 Continuous integration 을 도와주는 툴활용
	- 빌드, 테스트, 배포 등을 자동화 해서 생산성 증가
